...
//
		...
		if (cursor.moveToNext()) {//是某一门具体课程，那就也需要课程号
			course_name = cursor.getString(cursor.getColumnIndex("course_name"));

			//tb_courseType
			System.out.println(cursor.getString(cursor.getColumnIndex("course_type_id")));//it works
			course_type_name=cursor.getString(cursor.getColumnIndex("course_type_id"));
			Cursor cursor_1=db.rawQuery("select course_type_name from tb_CourseType where course_type_id=?",
					new String[]{cursor.getString(cursor.getColumnIndex("course_type_id"))});
			...
			if (cursor_6.moveToFirst()) {
				section = cursor_6.getString(cursor_6.getColumnIndex("section"));
			}
			//weekset   select * from tb_WeekSet where week_set_id=?
			Cursor cursor_7=db.rawQuery("select * from tb_WeekSet where week_set_id=?",
					new String[]{
			...
			cursor_8.moveToFirst();
			Cursor cursor_9=db.rawQuery("select * from tb_Building where building_id=?",
					new String[]{
							cursor_8.getString(cursor_8.getColumnIndex("building_id"))
					});
			if (cursor_9.moveToFirst()) {
				building_name = cursor_9.getString(cursor_9.getColumnIndex("building_name"));
			}
			Cursor cursor_10=db.rawQuery("select * from tb_Floor where floor_id=?",
					new String[]{
	...
	public ArrayList<HashMap<String, String>> getAllData() {
		SQLiteDatabase db = myhelper.getReadableDatabase();
		Cursor cursor = db.rawQuery("select * from tb_course where course_status_id='1'",new String[]{});
		//不开放和满额的不显示
		cursor.moveToFirst();
		// 生成动态数组，加入数据
		...
		for (int i = 0; i < cursor.getCount(); i++) {//改
			...
			String maxnum = cursor.getString(cursor.getColumnIndex("maxnum"));
	...
	public ArrayList<HashMap<String, String>> getPartOfData(String typeId) {
		SQLiteDatabase db = myhelper.getReadableDatabase();
		Cursor cursor = db.rawQuery("select * from tb_course where course_status_id='1' and course_type_id",new String[]{typeId});
		//不开放和满额的不显示
		cursor.moveToFirst();
		// 生成动态数组，加入数据
		ArrayList<HashMap<String, String>> listItem = new ArrayList<HashMap<String, String>>();
